...
public class MenuDelete : IAsyncMiddleware<long, bool>, IMiddlewarePlugin
{
    ...
    public MenuDelete()
    {
        Name = "Menu Delete Business Logic plugin";
        Version = "0.0.1";
        PluginId = Guid.NewGuid();
        Author = "Orun Innovations LLC";
        Description = "Deletes Menu from list ";
        ShortName = "Menu deletion";
        Enable = true;
        Level = 0;
        Dependencies = new List<Dependency>();
        EventCode = EventCodes.MenuDelete;
    }
    ...
    public async Task<bool> Run(long id, Func<long, Task<bool>> next)
    {
        try
        {
            ...
            if (!deleted)
            {
                Domain.Models.Menu entity = await _repository.GetOne(x => x.MenuId == id);
                if (Is.NullOrEmpty(entity))
                {
                    throw new Exception($"Menu: Entity with id {id} was not found"); ;
                }
