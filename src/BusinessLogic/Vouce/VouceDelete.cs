...
public class VouceDelete : IAsyncMiddleware<long, bool>, IMiddlewarePlugin
{
    ...
    public VouceDelete()
    {
        Name = "Vouce Delete Business Logic plugin";
        Version = "0.0.1";
        PluginId = Guid.NewGuid();
        Author = "Orun Innovations LLC";
        Description = "Deletes Vouce from list ";
        ShortName = "Vouce deletion";
        Enable = true;
        Level = 0;
        Dependencies = new List<Dependency>();
        EventCode = EventCodes.VouceDelete;
    }
    ...
    public async Task<bool> Run(long id, Func<long, Task<bool>> next)
    {
        try
        {
            ...
            if (!deleted)
            {
                Domain.Models.Vouce entity = await _repository.GetOne(x => x.VouceId == id);
                if (Is.NullOrEmpty(entity))
                {
                    throw new Exception($"Vouce: Entity with id {id} was not found"); ;
                }
