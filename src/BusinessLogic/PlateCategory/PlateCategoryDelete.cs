...
public class PlateCategoryDelete : IAsyncMiddleware<long, bool>, IMiddlewarePlugin
{
    ...
    public PlateCategoryDelete()
    {
        Name = "PlateCategory Delete Business Logic plugin";
        Version = "0.0.1";
        PluginId = Guid.NewGuid();
        Author = "Orun Innovations LLC";
        Description = "Deletes PlateCategory from list ";
        ShortName = "PlateCategory deletion";
        Enable = true;
        Level = 0;
        Dependencies = new List<Dependency>();
        EventCode = EventCodes.PlateCategoryDelete;
    }
    ...
    public async Task<bool> Run(long id, Func<long, Task<bool>> next)
    {
        try
        {
            ...
            if (!deleted)
            {
                Domain.Models.PlateCategory entity = await _repository.GetOne(x => x.PlateCategoryId == id);
