...
public class UserBenefitDelete : IAsyncMiddleware<long, bool>, IMiddlewarePlugin
{
    ...
    {
        Name = "UserBenefit Delete Business Logic plugin";
        Version = "0.0.1";
        PluginId = Guid.NewGuid();
        Author = "Orun Innovations LLC";
        Description = "Deletes UserBenefit from list ";
        ShortName = "UserBenefit deletion";
        Enable = true;
        Level = 0;
    ...
    public async Task<bool> Run(long id, Func<long, Task<bool>> next)
    {
        try
        {
            ...
            if (!deleted)
            {
                Domain.Models.UserBenefit entity = await _repository.GetOne(x => x.UserBenefitId == id);
                if (Is.NullOrEmpty(entity))
                {
                    throw new Exception($"UserBenefit: Entity with id {id} was not found"); ;
                }
