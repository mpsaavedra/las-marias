...
public class BenefitDelete : IAsyncMiddleware<long, bool>, IMiddlewarePlugin
{
    ...
    {
        Name = "Benefit Delete Business Logic plugin";
        Version = "0.0.1";
        PluginId = Guid.NewGuid();
        Author = "Orun Innovations LLC";
        Description = "Deletes Benefit from list ";
        ShortName = "Benefit deletion";
        Enable = true;
        Level = 0;
    ...
    public async Task<bool> Run(long id, Func<long, Task<bool>> next)
    {
        try
        {
            ...
            if (!deleted)
            {
                Domain.Models.Benefit entity = await _repository.GetOne(x => x.BenefitId == id);
