...
public class MovementDelete : IAsyncMiddleware<long, bool>, IMiddlewarePlugin
{
    ...
    {
        Name = "Movement Delete Business Logic plugin";
        Version = "0.0.1";
        PluginId = Guid.NewGuid();
        Author = "Orun Innovations LLC";
        Description = "Deletes Movement from list ";
        ShortName = "Movement deletion";
        Enable = true;
        Level = 0;
        Dependencies = new List<Dependency>();
        EventCode = EventCodes.MovementDelete;
    }
    ...
    public async Task<bool> Run(long id, Func<long, Task<bool>> next)
    {
        try
        {
            ...
            if (!deleted)
            {
                Domain.Models.Movement entity = await _repository.GetOne(x => x.MovementId == id);
