...
public class BrandDelete : IAsyncMiddleware<long, bool>, IMiddlewarePlugin
{
    ...
    public BrandDelete()
    {
        Name = "Brand Delete Business Logic plugin";
        Version = "0.0.1";
        PluginId = Guid.NewGuid();
        Author = "Orun Innovations LLC";
        Description = "Deletes Brand from list ";
        ShortName = "Brand deletion";
        Enable = true;
        Level = 0;
        Dependencies = new List<Dependency>();
        EventCode = EventCodes.BrandDelete;
    }
    ...
    public async Task<bool> Run(long id, Func<long, Task<bool>> next)
    {
        try
        {
            ...
            if (!deleted)
            {
                Domain.Models.Brand entity = await _repository.GetOne(x => x.BrandId == id);
