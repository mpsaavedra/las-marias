...
public class ProductPhotoDelete : IAsyncMiddleware<long, bool>, IMiddlewarePlugin
{
    ...
    {
        Name = "ProductPhoto Delete Business Logic plugin";
        Version = "0.0.1";
        PluginId = Guid.NewGuid();
        Author = "Orun Innovations LLC";
        Description = "Deletes ProductPhoto from list ";
        ShortName = "ProductPhoto deletion";
        Enable = true;
        Level = 0;
        Dependencies = new List<Dependency>();
        EventCode = EventCodes.ProductPhotoDelete;
    }
    ...
    public async Task<bool> Run(long id, Func<long, Task<bool>> next)
    {
        try
        {
            ...
            if (!deleted)
            {
                Domain.Models.ProductPhoto entity = await _repository.GetOne(x => x.ProductPhotoId == id);
                if (Is.NullOrEmpty(entity))
                {
                    throw new Exception($"ProductPhoto: Entity with id {id} was not found"); ;
                }
