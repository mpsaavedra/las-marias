    ...
    IAsyncMiddleware<CategoryCreateInputModel, Domain.Models.Category>, IMiddlewarePlugin
{
    ...
    {
        Name = "Category Create Business Logic plugin";
        Version = "0.0.1";
        ...
        EventCode = EventCodes.CategoryCreate;
    }
    ...
    )
    {
        try
        {
            ...
            Domain.Models.Category entity = await next(input);

            if (entity == null)
            {
                var data = _repository.Mapper.Map<Domain.Models.Category>(input);
