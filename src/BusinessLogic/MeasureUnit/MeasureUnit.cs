...
public class MeasureUnitDelete : IAsyncMiddleware<long, bool>, IMiddlewarePlugin
{
    ...
    {
        Name = "MeasureUnit Delete Business Logic plugin";
        Version = "0.0.1";
        PluginId = Guid.NewGuid();
        Author = "Orun Innovations LLC";
        Description = "Deletes MeasureUnit from list ";
        ShortName = "MeasureUnit deletion";
        Enable = true;
        Level = 0;
        Dependencies = new List<Dependency>();
        EventCode = EventCodes.MeasureUnitDelete;
    }
    ...
    public async Task<bool> Run(long id, Func<long, Task<bool>> next)
    {
        try
        {
            ...
            if (!deleted)
            {
                Domain.Models.MeasureUnit entity = await _repository.GetOne(x => x.MeasureUnitId == id);
