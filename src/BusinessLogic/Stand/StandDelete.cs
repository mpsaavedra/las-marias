...
public class StandDelete : IAsyncMiddleware<long, bool>, IMiddlewarePlugin
{
    ...
    public StandDelete()
    {
        Name = "Stand Delete Business Logic plugin";
        Version = "0.0.1";
        PluginId = Guid.NewGuid();
        Author = "Orun Innovations LLC";
        Description = "Deletes Stand from list ";
        ShortName = "Stand deletion";
        Enable = true;
        Level = 0;
        Dependencies = new List<Dependency>();
        EventCode = EventCodes.StandDelete;
    }
    ...
    public async Task<bool> Run(long id, Func<long, Task<bool>> next)
    {
        try
        {
            ...
            if (!deleted)
            {
                Domain.Models.Stand entity = await _repository.GetOne(x => x.StandId == id);
                if (Is.NullOrEmpty(entity))
                {
                    throw new Exception($"Stand: Entity with id {id} was not found"); ;
                }
